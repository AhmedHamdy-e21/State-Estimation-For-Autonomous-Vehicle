...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    delta_q=Quaternion(axis_angle=delta_x[2]).to_numpy()
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    p_est[k] = p_est[k-1] + delta_t * v_est[k-1] + (0.5 * delta_t**2) * ( q_est[k].to_mat() * imu_f[k-1] + g)

    v_est[k] = v_est[k-1] + delta_t * ( q_est[k-1].to_mat() * imu_f[k-1] + g)
